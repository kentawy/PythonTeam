...
# Завдання наступному студенту - створення функції для пошуку студента за прізвищем
 # Гаценко Максим, створення  функції для пошуку студента за прізвищем - search_student_by_last_name
def search_student_by_last_name(last_name):
        """
        Шукає студента за прізвищем.

        Parameters:
            last_name (str): Прізвище студента для пошуку.
        """
        found_students = {name: details for name, details in student_performance.items() if last_name.lower() in name.lower()}
        if found_students:
            print(f"Знайдені студенти з прізвищем '{last_name}':")
            for name, details in found_students.items():
                print(f"Студент: {name}, Група: {details['group_number']}, Курс: {details['course']}")
        else:
            print(f"Студентів з прізвищем '{last_name}' не знайдено.")

    # Функція для запиту прізвища студента для пошуку
def search_student_prompt():
        """
        Запитує у користувача прізвище студента для пошуку і викликає функцію пошуку.
        """
        last_name = input("Введіть прізвище студента для пошуку: ")
        search_student_by_last_name(last_name)
...
while True:
    print("\nКоманди: 'add' - додати студента, 'display' - показати список студентів, 'remove' - видалити студента, 'sort' - відсортувати студентів за середніми оцінками,'search' - пошук за прізвищем,'exit' - вихід")
